import pytest
from src.ft_linear_regression import FtLinearRegression as lr

@pytest.fixture
def model():
    """
    Fixture to create an instance of the FTLinearRegression class.
    """
    return lr(learning_rate=0.01, n_iterations=1000)

@pytest.fixture
def sample_data():
    """
    Fixture to provide sample data for testing.
    """
    return [[1, 2], [3, 4], [5, 6]]

def test_initialize(model):
    """
    Test the initialization of the FTLinearRegression class.
    """
    assert model.learning_rate == 0.01
    assert model.n_iterations == 1000
    assert model.theta0 == 0
    assert model.theta1 == 0
    assert model.data == []
    assert model.X_mean == 0.0
    assert model.X_std == 0.0
    assert model.y_mean == 0.0
    assert model.y_std == 0.0

def test_read_csv(model):
    """
    Test the read_csv method of the FTLinearRegression class.
    """
    data = model.read_csv("src/data.csv")
    assert len(data) > 0
    assert isinstance(data, list)

def test_normalize_data(model):
    """
    Test the __normalize_data method of the FTLinearRegression class.
    """
    X = [1, 3, 5]
    y = [2, 4, 6]
    X, y = model._FtLinearRegression__normalize_data(X, y)

    assert len(X) == 3
    assert len(y) == 3
    assert model.X_mean == 3.0
    assert model.X_std > 0
    assert model.y_mean == 4.0
    assert model.y_std > 0


def test_normalize_data_empty(model):
    """
    Test the __normalize_data method with empty data.
    """
    try:
        model._FtLinearRegression__normalize_data()
    except ValueError as e:
        assert str(e) == "Data must not be empty."

def test_normalize_data_invalid(model, sample_data):
    """
    Test the __normalize_data method with invalid data.
    """
    model = lr()
    model.data = sample_data
    try:
        model._FtLinearRegression__normalize_data(X=[1, 2], y=[3])  # Invalid lengths
    except ValueError as e:
        assert str(e) == "X and y must have the same length."

    model.data = [[1, 2], [3, 4], [5]]  # Invalid data structure
    try:
        model._FtLinearRegression__normalize_data()
    except ValueError as e:
        assert "but exactly 2 columns are required for X and y." in str(e)

def test_denormalize(model, sample_data):
    """
    Test the __denormalize method of the FTLinearRegression class.
    """
    model.data = sample_data
    model.y_mean = 4.0
    model.y_std = 2.0
    _, y_normalized = model._FtLinearRegression__normalize_data()
    y_denormalized = model._FtLinearRegression__denormalize(y_normalized)

    assert len(y_denormalized) == 3
    assert y_denormalized[0] == 2.0
    assert y_denormalized[1] == 4.0
    assert y_denormalized[2] == 6.0